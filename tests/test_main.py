"""Tests for plex_organizer.__main__ helper functions."""

from unittest.mock import patch, MagicMock
from pytest import mark, raises

from plex_organizer.__main__ import (
...
class TestDeleteUnwantedFiles:
    """Test _delete_unwanted_files from __main__."""
    ...
    def test_deletes_non_video_files(self, tmp_path):
        ...
        _delete_unwanted_files(str(tmp_path), ["video.mkv", "readme.txt", "info.nfo"])
    ...
    def test_deletes_sample_files(self, tmp_path):
        ...
        _delete_unwanted_files(str(tmp_path), ["sample.mkv", "real.mkv"])
    ...
    def test_preserves_index_files(self, tmp_path):
        ...
        _delete_unwanted_files(str(tmp_path), ["data.index"])
    ...
    def test_preserves_qb_temp_files(self, tmp_path):
        ...
        _delete_unwanted_files(str(tmp_path), ["file.mkv.!qB"])
    ...
    def test_preserves_script_temp_files(self, tmp_path):
        ...
        _delete_unwanted_files(str(tmp_path), ["video.langtag.mkv"])
        assert (tmp_path / "video.langtag.mkv").exists()


@mark.usefixtures("default_config")
class TestDeleteUnwantedDirectories:
    """Test _delete_unwanted_directories from __main__."""
...
class TestDeleteEmptyDirectories:
    """Test _delete_empty_directories from __main__."""
    ...
    def test_deletes_empty_subdirs(self, tmp_path):
        ...
        _delete_empty_directories(str(movies_dir))
...
class TestGetVideoFilesToProcess:
    ...
    def test_returns_only_video_files(self):
        ...
        result = _get_video_files_to_process("/media/movies", files, {})
    ...
    def test_excludes_indexed_files(self):
        ...
        result = _get_video_files_to_process("/media/movies", files, indexed)
...
class TestGetLock:
    ...
    @patch("plex_organizer.__main__.flock")
    @patch("plex_organizer.__main__.data_dir", return_value="/tmp/po")
    def test_acquires_lock_immediately(self, _mock_dd, mock_flock, tmp_path):
        """Lock acquired on the first try with no waiting."""
        with patch("plex_organizer.__main__.data_dir", return_value=str(tmp_path)):
            _get_lock()
        assert mock_flock.call_count >= 0

    @patch("plex_organizer.__main__.sleep")
    @patch("plex_organizer.__main__.log_debug")
    def test_retries_on_lock_contention(self, mock_debug, mock_sleep, tmp_path):
        ...
        with (
            patch("plex_organizer.__main__.data_dir", return_value=str(tmp_path)),
            patch("plex_organizer.__main__.flock", side_effect=flock_side_effect),
        ):
            _get_lock()
...
class TestAnalyzeVideoLanguages:
    ...
    @patch("plex_organizer.__main__.tag_audio_track_languages")
    @patch("plex_organizer.__main__.get_enable_audio_tagging", return_value=False)
    def test_skips_when_disabled(self, _mock_cfg, mock_tag):
        """No tagging occurs when audio tagging is disabled."""
        _analyze_video_languages("/media/movies", ["video.mkv"])
        mock_tag.assert_not_called()

    @patch("plex_organizer.__main__.tag_audio_track_languages")
    @patch("plex_organizer.__main__.get_enable_audio_tagging", return_value=True)
    ...
    def test_skips_plex_folder(self, _mock_cfg, mock_tag):
        ...
        _analyze_video_languages("/media/movies/Plex Versions", ["video.mkv"])
        mock_tag.assert_not_called()
    ...
    def test_skips_script_temp_files(self, _mock_cfg, mock_tag):
        ...
        _analyze_video_languages("/media/movies", ["video.langtag.mkv"])
        mock_tag.assert_not_called()
...
class TestDeleteUnwantedFilesErrors:  # pylint: disable=too-few-public-methods
    ...
    @patch("plex_organizer.__main__.log_error")
    def test_logs_oserror_on_remove(self, mock_log_error, tmp_path):
        ...
        with patch(
            "plex_organizer.__main__.remove", side_effect=OSError("perm denied")
        ):
            _delete_unwanted_files(str(tmp_path), ["readme.txt"])
...
class TestDeleteUnwantedDirErrors:  # pylint: disable=too-few-public-methods
    ...
    @patch("plex_organizer.__main__.log_error")
    @patch("plex_organizer.__main__.rmtree", side_effect=OSError("busy"))
...
class TestMoveDirectories:
    ...
    @patch("plex_organizer.__main__.index_root_for_path", return_value="/media/movies")
    @patch(
        "plex_organizer.__main__.movie_move",
    ...
    def test_moves_movie_and_indexes(self, mock_move, _ir, _si, mock_mark):
        ...
        _move_directories("/media/movies", "/media/movies/raw", ["film.mkv"])
    ...
    @patch("plex_organizer.__main__.mark_indexed")
    @patch("plex_organizer.__main__.should_index_video", return_value=False)
    @patch("plex_organizer.__main__.index_root_for_path", return_value="/media/movies")
    @patch("plex_organizer.__main__.movie_move", return_value="/media/movies/Film.mkv")
    ...
    @patch("plex_organizer.__main__.mark_indexed")
    @patch("plex_organizer.__main__.should_index_video", side_effect=OSError("fail"))
    @patch("plex_organizer.__main__.index_root_for_path", return_value="/media/movies")
    @patch("plex_organizer.__main__.movie_move", return_value="/media/movies/Film.mkv")
    ...
    @patch("plex_organizer.__main__.index_root_for_path")
    @patch("plex_organizer.__main__.movie_move")
    def test_skips_plex_folder(self, mock_move, mock_ir):
        """Plex-managed folders are skipped entirely."""
        _move_directories("/media/movies", "/media/movies/Plex Versions", ["video.mkv"])
    ...
    def test_skips_script_temp_files(self, mock_move, _ir, _si, _mark):
        ...
        _move_directories("/media/movies", "/media/movies/raw", ["video.langtag.mkv"])
        mock_move.assert_not_called()
...
class TestProcessDirectory:  # pylint: disable=too-few-public-methods
    ...
    @patch("plex_organizer.__main__._delete_empty_directories")
    @patch("plex_organizer.__main__._move_directories")
    @patch("plex_organizer.__main__._delete_unwanted_files")
    @patch("plex_organizer.__main__._analyze_video_languages")
    @patch("plex_organizer.__main__._get_video_files_to_process", return_value=[])
    @patch(
        "plex_organizer.__main__.walk", return_value=[("/media/movies", [], ["v.mkv"])]
    )
    @patch("plex_organizer.__main__.sync_subtitles_in_directory")
    @patch("plex_organizer.__main__.fetch_subtitles_in_directory")
    @patch("plex_organizer.__main__.merge_subtitles_in_directory")
    @patch(
        "plex_organizer.__main__.collect_indexed_videos",
...
class TestMain:
    ...
    @patch("plex_organizer.__main__._process_directory")
    @patch("plex_organizer.__main__._get_lock")
    @patch("plex_organizer.__main__.check_clear_log")
    @patch("plex_organizer.__main__.ensure_config_exists")
    @patch("plex_organizer.__main__.is_media_directory", return_value=True)
    @patch("plex_organizer.__main__.is_main_folder", return_value=True)
    @patch("plex_organizer.__main__.getuid", return_value=0)
    @patch("plex_organizer.__main__.remove_torrent")
    @patch("plex_organizer.__main__.log_debug")
    def test_main_folder_mode(
        ...
        with patch(
            ...
            return_value=MagicMock(start_dir="/media", torrent_hash="abc123"),
        ):
            main()
    ...
    @patch("plex_organizer.__main__.ensure_config_exists")
    @patch("plex_organizer.__main__.is_media_directory", return_value=True)
    @patch("plex_organizer.__main__.is_main_folder", return_value=False)
    @patch("plex_organizer.__main__.getuid", return_value=0)
    @patch("plex_organizer.__main__.log_debug")
    def test_single_directory_mode(
        ...
        with patch(
            "plex_organizer.__main__.ArgumentParser.parse_args",
            return_value=MagicMock(start_dir="/media/tv/Show", torrent_hash=None),
        ):
            main()
    ...
    @patch("plex_organizer.__main__.check_clear_log")
    @patch("plex_organizer.__main__.ensure_config_exists")
    @patch("plex_organizer.__main__.is_media_directory", return_value=False)
    @patch("plex_organizer.__main__.getuid", return_value=0)
    @patch("plex_organizer.__main__.log_debug")
    def test_non_media_directory_exits(
        ...
        with patch(
            "plex_organizer.__main__.ArgumentParser.parse_args",
            return_value=MagicMock(start_dir="/tmp/random", torrent_hash=None),
        ):
            main()
    ...
    @patch("plex_organizer.__main__.check_clear_log")
    @patch("plex_organizer.__main__.ensure_config_exists")
    @patch("plex_organizer.__main__.is_media_directory", side_effect=OSError("disk"))
    @patch("plex_organizer.__main__.getuid", return_value=0)
    @patch("plex_organizer.__main__.log_debug")
    def test_oserror_caught_and_logged(
        ...
        with patch(
            "plex_organizer.__main__.ArgumentParser.parse_args",
            return_value=MagicMock(start_dir="/media", torrent_hash=None),
        ):
            main()
    ...
    @patch("plex_organizer.__main__.check_clear_log")
    @patch("plex_organizer.__main__.ensure_config_exists")
    @patch("plex_organizer.__main__.getuid", return_value=1000)
    def test_non_root_exits_with_error(self, _uid, _cfg, _clr, _lock):
        """Non-root user gets an argument parser error."""
        with patch(
            "plex_organizer.__main__.ArgumentParser.parse_args",
            return_value=MagicMock(start_dir="/media", torrent_hash=None),
        ):
